"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.dateValidation = exports.passwordValidation = exports.emailValidation = void 0;
const UserErrorResponses_ts_1 = require("../../responses/errors/UserErrorResponses.ts");
const CommonErrorResponses_ts_1 = require("../../responses/errors/CommonErrorResponses.ts");
const emailValidation = (value, helpers) => {
    let regexEmail = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
    if (value.match(regexEmail)) {
        return value;
    }
    else {
        return helpers.message({ custom: UserErrorResponses_ts_1.UserErrorResponses.EMAIL_ERROR.message });
    }
};
exports.emailValidation = emailValidation;
const passwordValidation = (value, helpers) => {
    if (value) {
        if (value.length < 8) {
            return helpers.message({ custom: UserErrorResponses_ts_1.UserErrorResponses.PASSWORD_LENGTH_ERROR.message });
        }
        else {
            return value;
        }
    }
    else {
        return helpers.message({ custom: CommonErrorResponses_ts_1.CommonErrorResponses.REQUIRED.message });
    }
};
exports.passwordValidation = passwordValidation;
const dateValidation = (value, helpers) => {
    let regexDate = /^(?:(?:31(\/|-|\.)(?:0?[13578]|1[02]))\1|(?:(?:29|30)(\/|-|\.)(?:0?[13-9]|1[0-2])\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})$|^(?:29(\/|-|\.)0?2\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\d|2[0-8])(\/|-|\.)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:(?:1[6-9]|[2-9]\d)?\d{2})$/;
    if (value.match(regexDate)) {
        return value;
    }
    else {
        return helpers.message({ custom: UserErrorResponses_ts_1.UserErrorResponses.DATE_ERROR.message });
    }
};
exports.dateValidation = dateValidation;
