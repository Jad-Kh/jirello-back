...
export const removeUserFromCommunitySecurity = (
    res: IResponse,
    community: ICommunity,
    user: IUser,
    requestModel?: RemoveUserFromCommunityRequest,
): IResponse | boolean => {
    ...
    if (!community.userIds.includes(requestModel?.userId as string)) {
        return res
            .status(CommunityErrorResponses.COMMUNITY_USER_NOT_FOUND.code)
