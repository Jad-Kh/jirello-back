...
export const getCommunityUsersPaginatedHandler = async (req: IRequest<APISignature, "users">, res: IResponse, next: NextFunction): Promise<void> => {
    try {
        ...
        if (checkSecurity(getCommunityUsersPaginatedSecurity(res, community))) {
            ...
            const communityUsers = await UserQueries.getUsersOfCommunityPaginatedQuery(communityId, skip, limit);
